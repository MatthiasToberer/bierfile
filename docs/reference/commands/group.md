...
- **`bier group new <group> <mac>…`** makes a group of these Macs.
...
`bier vault group` is the same command by its old name, and
`bier group --drop` the old spelling of `drop`.
